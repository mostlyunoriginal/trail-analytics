...
def package(trail, destination):
    ...
    shutil.copytree(REPO/"viewer",out/"viewer",ignore=shutil.ignore_patterns("*.map"))
    ...
    report={"build_id":pointer["build_id"],"files":len(files),"bytes":sum(p.stat().st_size for p in files),"entry":"index.html","contains":"Viewer and one public-data release only; excludes raw caches, tools, notes and browser storage."}
...
if __name__=="__main__":
    ...
    a=p.parse_args();package(a.trail,a.out)
